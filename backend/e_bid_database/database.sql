...
-- count goes up by 1.
DELIMITER // CREATE PROCEDURE suspend_user(IN p_user_ID INT) BEGIN
...
SELECT count_suspended,
  VIP_status INTO current_suspended,
  current_VIP_status
FROM `user`
...
IF current_suspended + 1 >= 3 THEN -- Delete the user if count_suspended reaches 3
DELETE FROM `user`
WHERE user_ID = p_user_ID;
END IF;
END // DELIMITER;
